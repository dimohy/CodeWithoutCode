...
**기능 1: 인라인 제안 (Inline Suggestions)**
...
**단축키**: `Ctrl + I` (Windows) 또는 `Cmd + I` (Mac)

**기능 3: 코드 설명 (Explain Code)**
...
2. **Chat 패널 열기**
   - 사이드바에서 채팅 아이콘 클릭
   - 또는 `Ctrl + Alt + I` (Windows)

3. **인라인 Chat 단축키**
   - `Ctrl + I`: 현재 위치에서 AI와 대화
...
### 실습 2: Chat 모드로 대화하며 개발하기

**목표**: AI와 대화하며 문제를 점진적으로 해결하기

**실습 과정:**

1. **새 파일 생성**: `file_organizer.py`

2. **Copilot Chat 열기** (`Ctrl + I`)
...
```python
# 할일 관리 애플리케이션
# 할일 클래스: 제목, 우선순위, 마감일, 완료여부
# TodoManager 클래스: 할일들을 관리

# 기능:
# 1. add_todo(title, priority, due_date): 할일 추가
# 2. list_todos(): 모든 할일 표시
# 3. complete_todo(index): 할일 완료 표시
# 4. delete_todo(index): 할일 삭제
# 5. save_to_file(filename): JSON으로 저장
# 6. load_from_file(filename): JSON에서 불러오기
```

Enter를 누르고 Copilot이 클래스 구조를 생성하게 합니다.
...
# 5. 종료
# 
# 사용자 입력을 받아서 해당 기능 실행
```

**4단계: 테스트 및 개선**

프로그램을 실행하고 각 기능을 테스트합니다. 문제가 발견되면:

```
Copilot Chat에 요청:
"할일을 삭제할 때 잘못된 인덱스를 입력하면 
오류가 발생해. 이걸 처리해줘"
```
...
```
"마감일이 오늘인 할일들만 보여주는 기능 추가해줘"
"우선순위가 높은 순서로 정렬해서 보여줘"
"완료된 할일은 다른 색으로 표시하고 싶어"
```
