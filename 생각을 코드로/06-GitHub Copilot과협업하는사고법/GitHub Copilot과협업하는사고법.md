...
4. 파란색 "Install" 버튼 클릭
5. 설치가 완료되면 "Sign in to GitHub" 버튼이 나타남
6. 버튼을 클릭하여 GitHub 계정으로 로그인
7. 로그인 후 VS Code로 돌아오면 설정 완료!

**Step 3: Copilot이 활성화되었는지 확인**

1. VS Code 오른쪽 하단을 보세요
2. Copilot 아이콘(비행기 모양 또는 로고)이 보이면 성공
3. 아이콘에 마우스를 올리면 "GitHub Copilot is active" 메시지 표시

// 이미지로 교체되어야 함 : VS Code 화면에서 Copilot 확장 설치 과정을 보여주는 스크린샷. Extensions 검색창, GitHub Copilot 확장, Install 버튼이 표시됨
프롬프트: Screenshot-style illustration of VS Code interface showing Extensions sidebar with "GitHub Copilot" search result, blue Install button, and GitHub login prompt. Clean UI mockup style, highlighting the key buttons with arrows or circles
